...
extern double kernel(unsigned n, const double a[n][n], const double b[n][n]);

// Adjusted for the new initialization function
extern void init_array(int n, float a[n][n]) {
   int i, j;
...
int main(int argc, char *argv[]) {
   ...
   for (m = 0; m < NB_METAS; m++) {
      ...
      /* allocate arrays */
      double (*a)[size] = malloc(size * sizeof(double[size]));
      double (*b)[size] = malloc(size * sizeof(double[size]));
      ...
      /* init arrays */
      srand(0);
      init_array(size, (float(*)[size])a);
      init_array(size, (float(*)[size])b);
   ...
   const uint64_t nb_inner_iters = size * size; // Adjusted for each kernel
   int i;
   for (i = 0; i < repm; i++) {
      printf("Instance %u/%u\n", i + 1, repm);
